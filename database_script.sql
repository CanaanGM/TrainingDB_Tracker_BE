...
create table if not exists user_exercise(
        ...
        -- how many times the exercise was logged 'used', count of unique exercise name in list
        -- for bodyBuilding type exercises
        best_weight real not null default 0,
        -- the largest weight ever, MAX
        average_weight real not null default 0,
        -- the AVG weight of overall records used
        last_used_weight_kg real not null default 0,
        -- should be in a history table, but this is fine for now
...
create table if not exists exercise_record (
        ...
        -- you always did at least one, hence u created a record
        mood integer not null default 5,
        -- 1 ~ 10
        timer_in_seconds integer default null,
        weight_used_kg real not null default 0,
        -- 0 is body weight
        ...
        CHECK (
                rate_of_perceived_exertion >= 0
                AND rate_of_perceived_exertion <= 10
        ),
...
create table if not exists block_exercises(
        ...
        instructions text,
        repetitions integer,
        -- if it has a rep range to go thru
        timer_in_seconds integer,
        -- for timed sets
        distance_in_meters integer,
        -- for walking bullshit exercises, like walking lunges
        created_at datetime default current_timestamp,
